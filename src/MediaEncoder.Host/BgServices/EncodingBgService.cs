...
namespace MediaEncoder.Host
{
    public class EncodingBgService : BackgroundService
    {
        ...
        public EncodingBgService(IServiceScopeFactory serviceScope)
        {
            //MEDbContext等是Scoped，而BackgroundService是Singleton，所以不能直接注入，需要手动开启一个新的Scope
            this._serviceScope = serviceScope.CreateScope();
            var sp = _serviceScope.ServiceProvider;
            _dbContext = sp.GetRequiredService<MediaDbContext>();
            ...
            this._logger = sp.GetRequiredService<ILogger<EncodingBgService>>();
            this._httpClientFactory = sp.GetRequiredService<IHttpClientFactory>();
        ...
        private async Task<(bool ok, FileInfo sourceFile)> DownloadSrcAsync(EncodingItem encItem, CancellationToken cancellationToken)
        {
            //开始下载源文件
            string tempDir = Path.Combine(Path.GetTempPath(), "MediaEncodingDir");
            //源文件的临时保存路径
            string sourceFullPath = Path.Combine(tempDir, Guid.NewGuid() + "."
                + Path.GetExtension(encItem.FileName));
            FileInfo sourceFile = new FileInfo(sourceFullPath);
            long id = encItem.Id;
            sourceFile.Directory!.Create();//创建可能不存在的文件夹
            _logger.LogInformation($"Id={id}，准备从{encItem.SourceUrl}下载到{sourceFullPath}");

            HttpClient _httpClient = _httpClientFactory.CreateClient();
            var statusCode = await _httpClient.DownloadFileAsync(encItem.SourceUrl, sourceFullPath, cancellationToken);
            if (statusCode != HttpStatusCode.OK)
            {
                _logger.LogWarning($"下载Id={id}，Url={encItem.SourceUrl}失败，{statusCode}");
                sourceFile.Delete();
            ...
            string outputFormat, CancellationToken ct)
        {
            ...
            {
                _logger.LogError($"转码失败{ex.Message}");
                return false;
            }
            return true;
        }
        ...
        private async Task ProcessItemAsync(EncodingItem encItem, CancellationToken ct)
        {
            ...
            encItem.Start();
            await _dbContext.SaveChangesAsync(ct);//立即保存一下状态的修改
                                                 //发出一次集成事件
            ...
            try
            {
                _logger.LogInformation($"下载Id={id}成功，开始计算Hash值");
                long fileSize = srcFile.Length;
                string srcFileHash = ComputeSha256Hash(srcFile);
                //如果之前存在过和这个文件大小、hash一样的文件，就认为重复了
                var prevInstance = await repository.FindCompelteItemAsync(srcFileHash, fileSize);
                if (prevInstance != null)
                {
                    _logger.LogInformation($"检查Id={id}Hash值成功，发现已经存在相同大小和Hash值的旧任务Id={prevInstance.Id}，返回！");
                ...
                //开始转码
                _logger.LogInformation($"Id={id}开始转码，源路径:{srcFile},目标路径:{destFile}");
                string outputFormat = encItem.OutputFormat;
                ...
                //开始上传
                _logger.LogInformation($"Id={id}转码成功，开始准备上传");
                Uri destUrl = await UploadFileAsync(destFile, ct);
                encItem.Complete(destUrl.AbsoluteUri);
                encItem.ChangeFileMeta(fileSize, srcFileHash);
                _logger.LogInformation($"Id={id}转码结果上传成功");
                //发出集成事件和领域事件
            }
            finally
            {
        ...
        protected override async Task ExecuteAsync(CancellationToken ct = default)
        {
            while (!ct.IsCancellationRequested)
            {
                ...
                var readyItems = await repository.GetListAsync(ItemStatus.Ready);
